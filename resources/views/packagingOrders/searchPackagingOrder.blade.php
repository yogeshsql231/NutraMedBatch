...
<form action="{{ route('updatePackagingOrder',['packagingOrder'=>$editData->id]) }}" method="POST">
    ...
    <div class="container">
        ...
        @php
        $testDecodeing= json_decode($editData->testSerial);
        @endphp

        {{-- dynamic text box --}}
        <div class="row mt-2" id="dynamicInputContainer">
            @foreach (old('testSerial', $testDecodeing ?? []) as $index => $testSerial)
            <div class="col-12 col-md-4 mb-2 d-flex align-items-center input-group">
                <label for="test" class="form-label me-1">Test {{ $index + 1 }}</label>
                <input type="text" name="test[{{ $index }}]" class="form-control me-2" value="{{ $testSerial }}">
